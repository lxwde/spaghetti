<template>
  <div>
    <h2 id="page-heading" data-cy="DepartmentHeading">
      <span v-text="$t('vuerealApp.department.home.title')" id="department-heading">Departments</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('vuerealApp.department.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'DepartmentCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('vuerealApp.department.home.createLabel')"> Create a new Department </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && departments && departments.length === 0">
      <span v-text="$t('vuerealApp.department.home.notFound')">No departments found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="departments">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('vuerealApp.department.departmentName')">Department Name</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="vuerealApp.department.delete.question" data-cy="departmentDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-department-heading" v-text="$t('vuerealApp.department.delete.question', { id: removeId })">

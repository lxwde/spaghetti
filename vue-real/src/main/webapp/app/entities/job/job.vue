<template>
  <div>
    <h2 id="page-heading" data-cy="JobHeading">
      <span v-text="$t('vuerealApp.job.home.title')" id="job-heading">Jobs</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('vuerealApp.job.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'JobCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-job">
            ...
            <span v-text="$t('vuerealApp.job.home.createLabel')"> Create a new Job </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && jobs && jobs.length === 0">
      <span v-text="$t('vuerealApp.job.home.notFound')">No jobs found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('jobTitle')">
              <span v-text="$t('vuerealApp.job.jobTitle')">Job Title</span>
            ...
            <th scope="row" v-on:click="changeOrder('minSalary')">
              <span v-text="$t('vuerealApp.job.minSalary')">Min Salary</span>
            ...
            <th scope="row" v-on:click="changeOrder('maxSalary')">
              <span v-text="$t('vuerealApp.job.maxSalary')">Max Salary</span>
            ...
            <th scope="row" v-on:click="changeOrder('employee.id')">
              <span v-text="$t('vuerealApp.job.employee')">Employee</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="vuerealApp.job.delete.question" data-cy="jobDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-job-heading" v-text="$t('vuerealApp.job.delete.question', { id: removeId })">

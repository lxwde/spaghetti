...
class DummymongoKafkaResourceIT {
    ...
    void setup() {
        ...
        DummymongoKafkaResource kafkaResource = new DummymongoKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/dummymongo-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/dummymongo-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())

...
class DummyredisKafkaResourceIT {
    ...
    void setup() {
        ...
        DummyredisKafkaResource kafkaResource = new DummyredisKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/dummyredis-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/dummyredis-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
